...
class Solution extends Algorithm{
    Long[][] dp;
    int[] xPrime = {0, 1, 4, 6, 8, 9, 10, 12, 14, 15,  16, 18};
    double a, b;
    ...
    public Solution() {
        ...
        for (int i = 0; i < xPrime.length; i++) {
            for (int j = 0; j < xPrime.length; j++) {
                result += combination(18, xPrime[i]) * Math.pow(a, xPrime[i]) * Math.pow((1 - a), 18 - xPrime[i])
                        * combination(18, xPrime[j]) * Math.pow(b, xPrime[j]) * Math.pow((1 - b), 18 - xPrime[j]);
            }
        }
